...
- **Every KR is a real metric.** It maps to a number on a [dashboard](../post-launch/dashboards.md) with an
...
- **Aligned, not dictated.** Departments propose OKRs that ladder up to the company north-star; the
  [CEO Agent](../orchestration/ceo-agent.md) resolves conflicts.
- **Learn, don't hide.** Misses feed the [Continuous Improvement Engine](../orchestration/continuous-improvement-engine.md);
  decisions are recorded in [decision memory](../memory/decision-memory.md).
...
[engineering](../engineering/metrics.md), [product](../product/metrics.md), [sales](../sales/metrics.md),
[customer-success](../customer-success/metrics.md). OKR Key Results are drawn from these, so goals and
