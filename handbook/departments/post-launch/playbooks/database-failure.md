...
4. **If corruption:** isolate, then restore from the latest verified [backup](failed-backup.md); quantify data loss window.
5. **Verify** integrity before resuming writes.
6. **Root cause** with the [Database Maintenance Agent](../../../../ai/agents/post-launch/database-maintenance-agent.md).

**Success criteria:** data integrity preserved; service restored from a known-good state.
**Escalate if:** data loss is possible/confirmed → human approval before destructive recovery ([approval](../../../../ai/orchestration/approval-engine.md)).
