...
- Constrain each agent to the tools in its [spec](../agents/agent-template.md); least privilege.
- Never place secrets or personal data in prompts, logs, or [memory](../memory/retention-and-versioning.md) that isn't access-controlled.
- Test with adversarial prompts and jailbreak attempts ([evaluation-and-testing.md](evaluation-and-testing.md)).
- Human-in-the-loop for high-risk actions ([approval engine](../orchestration/approval-engine.md)).

Findings feed the [Security Reviewer](../orchestration/security-reviewer.md) and, in production, the
